...
class JPU(nn.Module):
    ...
    def forward(self, inputs):
        ...
        feats[-2] = F.interpolate(
            feats[-2], size=(h, w), mode="bilinear", align_corners=True
        )
        feats[-3] = F.interpolate(
            feats[-3], size=(h, w), mode="bilinear", align_corners=True
        )
...
class Head(nn.Module):
    def __init__(self, classify_classes, norm_layer=nn.BatchNorm2d, bn_momentum=0.0003):
        ...
        self.classify = nn.Conv2d(
            in_channels=256,
            out_channels=self.classify_classes,
            kernel_size=1,
            stride=1,
            padding=0,
            dilation=1,
...
class FastFCN(nn.Module):
    def __init__(self, n_class, bn_momentum=0.01):
        super(FastFCN, self).__init__()
        self.Resnet101 = get_resnet101(
            dilation=[1, 1, 1, 2], bn_momentum=bn_momentum, is_fpn=True
        )
        self.jpu = JPU([512, 1024, 2048], width=512, norm_layer=nn.BatchNorm2d)
        self.head = Head(n_class, norm_layer=nn.BatchNorm2d)

    def forward(self, input):
        blocks = self.Resnet101(input)
        f1, f2, f3, f4 = self.jpu(blocks)
        pred = self.head([f1, f2, f3, f4])

        output = F.interpolate(
            pred, size=input.size()[2:4], mode="bilinear", align_corners=True
        )
        return output
...
def main():
    ...
    net = FastFCN(class_num=num_classes)
    out = net(x)
    print(out.shape)
    # summary(net, input_size=(inchannel, in_h, in_w))
